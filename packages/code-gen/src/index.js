...
function generateNodeComponent(node, options) {
    const baseStyles = {
        position: 'absolute',
        left: `${node.x}px`,
        top: `${node.y}px`,
...
export function generateReactTailwind(canvas, options = {}) {
...
export function generateHTMLCSS(canvas, options = {}) {
    const opts = { ...defaultOptions, ...options };
    let html = '';
    let css = '';
    html += `<!DOCTYPE html>\n<html lang="en">\n<head>\n`;
...
export function generateCSSVariables(canvas) {
    let css = ':root {\n';
...
export function generateTailwindConfig(canvas) {
    const config = {
        theme: {
            extend: {},
        },
